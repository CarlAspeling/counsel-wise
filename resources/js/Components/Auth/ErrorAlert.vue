<template>
    ...
    >
        <div
            v-if="show"
            class="mb-4 p-4 rounded-md border-l-4 border-red-400 bg-red-50"
            role="alert"
                ...
                <div class="ml-3 flex-1">
                    ...
                    <div class="text-sm text-red-700" :class="{ 'mt-2': title }">
                        ...
                        <div v-else-if="typeof message === 'object'">
                            <div v-for="(errors, field) in message" :key="field" class="mb-2">
                                <strong class="font-medium">{{ field }}:</strong>
                                <ul v-if="Array.isArray(errors)" class="list-disc list-inside ml-4">
                                    <li v-for="(error, index) in errors" :key="index">
                                        {{ error }}
                                    </li>
                                </ul>
                                <span v-else class="ml-1">{{ errors }}</span>
                            </div>
                        </div>
                    </div>
                </div>
